...
# Bulk-create hybrid users on-premises using Excel import
write-host "Creating user(s) from Excel file"
Import-Excel GXPimport.xlsx | ForEach-Object { ./New-HybridUser.PS1 -TemplateUser $_.TemplateUser -UserAlias $_.UserAlias -DisplayName $_.DisplayName -FirstName $_.FirstName -LastName $_.LastName -UPN $_.UPN -Password (ConvertTo-SecureString -String $_.Password -AsPlainText -Force) -OU $_.OU }
...
# Bulk-assign licenses in Office 365 using Excel import
write-host "Assigning user licenses and activating the mailboxes"cel GXPimport.xlsx | ForEach-Object { ./Assign-License.PS1 -UPN $_.UPN -TemplateUser $_.TemplateUser }